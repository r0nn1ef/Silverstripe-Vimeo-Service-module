...
class VimeoService extends RestfulService
{
    ...
    public function errorCatch($response)
    {
        ...
        if (strpos($err_msg, '<') === false) {
            user_error("VimeoService Error: $err_msg", E_USER_ERROR);
        }
     ...
     */
    public function getVideoFeed($method=null, $call_params=array(), $page_limit=10)
    {
     ...
     */
    public function getSingleVideo($call_params=array())
    {
        if (!self::$api_key || ! self::$api_secret_key) {
            user_error('Fatal error: Invalid API keyand/or secret specified!');
            return false;
